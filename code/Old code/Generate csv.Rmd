...
for(event in events){
  ...
  for(team in teams){
    ...
    for(map in maps){
      ...
      write.csv(temp[,c("attackerX","attackerY","victimSide","victimX","victimY","attackerName","victimName","weapon")],paste("D3/csv",event,team,map,paste(event,team,map,"CT","csv",sep="."), sep="/"), row.names = FALSE, quote=FALSE)
      
      temp <- heatmap %>%  filter(map_name==map) %>% filter(attackerSide=="T")
      temp <- temp[complete.cases(temp), ]
      write.csv(temp[,c("attackerX","attackerY","victimSide","victimX","victimY","attackerName","victimName","weapon")],paste("D3/csv",event,team,map,paste(event,team,map,"T","csv",sep="."), sep="/"), row.names = FALSE, quote=FALSE)
    }
  }
}
