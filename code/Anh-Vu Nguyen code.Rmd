...
heat_kills <- kills %>%  filter(map_name == "de_mirage") %>% filter(match_event=="StarLadder Major Berlin 2019" )
library(ggpubr)
library(ggimage)
img <- readJPEG("maps/de_mirage_light.jpeg")

# Area + contour
ggplot(heat_kills, aes(x=attackerX, y=attackerY) )+
background_image(img,alpha=0.5) +
#geom_point(alpha=0.2,color="red")+
stat_density_2d(aes(fill = ..level..), geom = "polygon",alpha=0.5,h=500)+ scale_fill_continuous(type = "viridis")+
  xlim(-3050, 1700)+
