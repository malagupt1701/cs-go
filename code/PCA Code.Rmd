...
```{r}
team_killsstat.pca <- prcomp(team_killsstat[,c(3,4,6:10,16:22,26)], center = TRUE,scale. = TRUE)
...
We rescale on the center of the graph (ignoring the one very far outliers) and color points so that teams that have won appear in a certain color, and teams that didn't appear in another color.


```{r}
team_killsstat$win <- ifelse(team_killsstat$relative_score_diff > 0, "win", "lose") 
#This columns tells if the team won (1) or lost (0)
```

```{r}

p <- ggbiplot(team_killsstat.pca, obs.scale = 1, var.scale = 1,circle=TRUE, groups=team_killsstat$win,varname.adjust = 1.45, alpha = 0.5)
p <- p + xlim(-4.9, 3) + ylim(-3.1, 5)
p
```

```{r}
p2 <- ggbiplot(team_killsstat.pca, obs.scale = 2, var.scale = 0.1, varname.adjust = 1, alpha = 0)
p2 <- p2 + xlim(-5, 3) + ylim(-5, 3)
p2
```
```
```{r}
